document.addEventListener('DOMContentLoaded', async () => {
  ...
  const loadUpgradeState = async () => {
    try {
      const [up1Bought, up2Bought, up3Bought, up4Bought, up5Bought] = await Promise.all([
      ...
      return { up1Bought, up2Bought, up3Bought, up4Bought, up5Bought };
    } catch (e) {
      console.error("Error loading upgrade states:", e);
      return { up1Bought: new Decimal(0), up2Bought: new Decimal(0), up3Bought: new Decimal(0), up4Bought: new Decimal(0), up5Bought: new Decimal(0) };
    }
  };
...
const addCPS = (upgradeKey, number, cost) => {
  ...
  const initializeGame = async () => {
    try {
      const [{ clickCount, cps, lastTime }, { up1Bought, up2Bought, up3Bought, up4Bought, up5Bought }] = await Promise.all([
        loadGameState(),
        loadUpgradeState()
      ]);
      gameState.clickCount = clickCount;
